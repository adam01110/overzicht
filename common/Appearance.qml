...
Singleton {
    ...
    colors: QtObject {
        property color accent: root.themeValue(["accent"], root.themeValue(["secondary"], "#D5C0D7"))
        property color border: root.themeValue(["border"], root.themeValue(["outlineVariant"], "#4C444D"))
        property color outline: root.themeValue(["outline"], "#988E97")
        property color panel: root.themeValue(["panel"], root.themeValue(["background"], "#161217"))
        property color panelText: root.themeValue(["panelText"], root.themeValue(["onBackground"], "#EAE0E7"))
        property color shadow: ColorUtils.transparentize(root.themeValue(["shadow"], "#000000"), 0.7)
        property color tooltip: root.themeValue(["tooltip"], root.themeValue(["inverseSurface"], "#EAE0E7"))
        property color tooltipText: root.themeValue(["tooltipText"], root.themeValue(["inverseOnSurface"], "#342F34"))
        property color window: root.themeValue(["window"], root.themeValue(["surfaceContainer"], "#231E23"))
        property color windowText: root.themeValue(["windowText"], root.themeValue(["onSurface"], "#EAE0E7"))
        property color workspace: root.themeValue(["workspace"], root.themeValue(["surfaceContainerLow"], "#1F1A1F"))
        property color workspaceText: root.themeValue(["workspaceText"], root.themeValue(["onSurfaceVariant"], "#CFC3CD"))

        readonly property color panelBorder: ColorUtils.mix(border, panel, 0.4)
        readonly property color tooltipBorder: ColorUtils.transparentize(tooltipText, 0.7)
        readonly property color windowActive: ColorUtils.mix(window, windowText, 0.80)
        readonly property color windowBorder: ColorUtils.transparentize(outline, 0.7)
        readonly property color windowHover: ColorUtils.mix(window, windowText, 0.90)
        readonly property color workspaceHover: ColorUtils.mix(workspace, workspaceText, 0.92)
    }
